...
# Script Title: OSCP Machine Setup and Enumeration Helper
# Date: 06/23/2024
# Author: Robboman
...
if [ $# -lt 4 ]; then
    ...
    printf "${red}Example: oscp exam exam1 sa1 yes 192.168.1.143${nc}\n"
    printf "${red}Example: oscp exam exam1 ad_nonmap no 192.168.1.140${nc}\n"
    exit 1
fi
...
dir_kalitools="/home/kali/Tools"
dir_kaliscripts="/home/kali/Scripts"
dir_kalibackup="/home/kali/Backup"
dir_kalictf="/home/kali/CTF"
dir_oscp="/home/kali/OSCP"
dir_type="$dir_oscp/$type"
dir_name="$dir_type/$name"
if [ "$boundary" == "ad_nonmap" ]; then
    dir_labboundary="$dir_name/ad_int"
else
    dir_labboundary="$dir_name/$boundary"
fi
dir_exp="$dir_labboundary/exploits"
...
if [ ! -d "$dir_labboundary" ]; then
    mkdir -p $dir_labboundary
fi
if [ ! -d "$dir_exp" ]; then
    mkdir -p $dir_exp
fi
...
if [ "$boundary" == "ad_nonmap" ]; then
    exec > >(tee $dir_name/$name-ad_nonmap.txt) 2>&1
else
    exec > >(tee $dir_name/$name-$boundary.txt) 2>&1
fi
...
#Print IPs provided
print_header "IP INPUT" "${bold_white}" "true"
printf "$pre_info IP Addresses Provided:\n${magenta}$arg_ip_list${nc}\n"

# Enumerate each IP address
for ip in $arg_ip_list; do
    ttl=$(ping -c 1 $ip 2>/dev/null | grep -i ttl | cut -d ' ' -f 6 | cut -d '=' -f 2)
    if [[ -n "$ttl" ]]; then
        os=$(determine_os $ttl)
    fi

    # Extract the last octet of the IP address
    ip_last=$(echo $ip | cut -d '.' -f 4)
    ...
    # Establish subshell for each IP address
    (

        # Write IP script output to file
        if [ "$boundary" == "ad_nonmap" ]; then
            exec > >(tee $dir_labboundary/ad_nonmap-$ip_last.txt) 2>&1
        else
            exec > >(tee $dir_labboundary/$boundary-$ip_last.txt) 2>&1
        fi

        # Print machine characteristics
        print_header "MACHINE CHARACTERISTICS" "${bold_white}" "true"
        printf "$pre_info Machine/Lab Name: ${magenta}${name^}${nc}\n"
        printf "$pre_info Operating System: ${magenta}$os${nc}\n"
        if [ "$boundary" == "ad_int" ] || [ "$boundary" == "ad_nonmap" ]; then
            printf "$pre_info Boundary/Subnet: ${magenta}Active Directory Internal${nc}\n"
        elif [ "$boundary" == "ad_ext" ]; then
            printf "$pre_info Boundary/Subnet: ${magenta}Active Directory External${nc}\n"
        else
            sa_num=$(echo "$boundary" | grep -oE '[0-9]+')
            printf "$pre_info Boundary/Subnet: ${magenta}Standalone $sa_num${nc}\n"
        fi
        printf "$pre_info IP Address: ${magenta}$ip${nc}\n"
        printf "$pre_info File Suffix: ${magenta}$ip_last${nc}\n"
        ...
        if [ "$boundary" == "ad_nonmap" ]; then
            printf "$pre_info Type: ${bold_magenta}None${nc}\n"
            printf "$pre_info Checking to see if file nmap_open_tcp_$ip_last exists!${nc}\n"
            if [ -s "$dir_nmap/nmap_open_tcp_$ip_last" ]; then
                printf "$pre_win File nmap_open_tcp_$ip_last exists! Moving on.${nc}\n"
            else
                printf "$pre_fail File nmap_open_tcp_$ip_last does not exist or is empty!${nc}\n"
                printf "$pre_info Copy and paste internal scan results into $dir_nmap/nmap_open_tcp_$ip_last and run again!${nc}\n"
                sudo touch $dir_nmap/nmap_open_tcp_$ip_last
                exit 1
            fi
        ...
        if [ "$boundary" == "ad_nonmap" ]; then
            printf "$pre_info Type: ${bold_magenta}None${nc}\n"
            printf "$pre_info Note: This scan should be considered optional unless you have root rights in pivot machine.${nc}\n"
            printf "$pre_info Checking to see if file nmap_open_udp_$ip_last exists!${nc}\n"
            if [ ! -s "$dir_nmap/nmap_open_udp_$ip_last" ]; then
        ...
        if [ "$boundary" == "ad_int" ] || [ "$boundary" == "ad_ext" ] || [ "$boundary" == "ad_nonmap" ]; then
        ...
        if [ "$boundary" == "ad_int" ] || [ "$boundary" == "ad_ext" ] || [ "$boundary" == "ad_nonmap" ]; then
            domain_multi_nmap=$(grep -oPi '[a-zA-Z0-9-]+\.oscp\.exam' "$dir_nmap/nmap_scan_tcp_$ip_last" | sort -u | awk '{print tolower($0)}')
            domain_nmap=$(grep -oPi '[a-zA-Z0-9-]+\.oscp\.exam' "$dir_nmap/nmap_scan_tcp_$ip_last" | sort -u | head -n 1 | awk '{print tolower($0)}')
            fqdn_nmap=$(grep -oPi '([a-zA-Z0-9-]+\.){1,}[a-zA-Z0-9-]+\.oscp\.exam' "$dir_nmap/nmap_scan_tcp_$ip_last" | sort -u | head -n 1 | awk '{print tolower($0)}')
        else
        ...
        printf "$pre_info OffSec FQDN: ${magenta}$name.offsec${nc}\n"
        ...
        if [ "$boundary" == "ad_nonmap" ]; then
            echo "$ip $fqdn" >> $dir_name/hosts_internal
        else
            echo "$ip $fqdn" >> $dir_name/hosts_$boundary
        fi
...
tree -A -C --dirsfirst $dir_labboundary
printf "${yellow}cd $dir_labboundary${nc}\n"
